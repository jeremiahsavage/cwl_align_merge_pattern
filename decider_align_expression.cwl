...
expression: |
   ${
      ...
      function get_slice_number(fastq_name) {
        if (endsWith(fastq_name, '_1.fq')) {
          return -5
        }
        else if (endsWith(fastq_name, '_s.fq')) {
          return -5
        }
        else if (endsWith(fastq_name, '_o1.fq')) {
          return -6
        }
        else if (endsWith(fastq_name, '_o2.fq')) {
          return -6
        }
        else {
          exit()
      ...
      for (var i = 0; i < inputs.fastq_path.length; i++) {
        ...
        var readgroup_name = fq_name.slice(0,slice_number) + ".readgroup";
